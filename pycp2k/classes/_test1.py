...
class _test1(InputSection):
    def __init__(self):
        ...
        self._repeated_subsections = {'EIGENSOLVER': '_eigensolver1', 'PW_TRANSFER': '_pw_transfer1', 'CP_FM_GEMM': '_cp_fm_gemm1', 'CP_DBCSR': '_cp_dbcsr1', 'DBM': '_dbm1'}
        self._aliases = {'Clebsch': 'Clebsch_gordon'}
        self._attributes = ['EIGENSOLVER_list', 'PW_TRANSFER_list', 'CP_FM_GEMM_list', 'CP_DBCSR_list', 'DBM_list']
    ...
    def DBM_add(self, section_parameters=None):
        new_section = _dbm1()
        if section_parameters is not None:
            if hasattr(new_section, 'Section_parameters'):
                new_section.Section_parameters = section_parameters
        self.DBM_list.append(new_section)
        return new_section
